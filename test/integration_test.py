...
from responses import activate, add, POST

from rainbird import (
    Controller,
    responses,
    DeviceInfo,
    StationStates,
...
from rainbird.encryption import encrypt

MOCKED_RAINBIRD_HOST = "rainbird.local"
...
class TestCase(unittest.TestCase):
    ...
    def test_get_model_and_version(self):
        ...
        rainbird = Controller(MOCKED_RAINBIRD_HOST, MOCKED_PASSWORD)
        self.assertEqual(
            DeviceInfo(16, 1, 3), rainbird.get_model_and_version()
        )
    ...
    def test_get_available_stations(self):
        ...
        rainbird = Controller(MOCKED_RAINBIRD_HOST, MOCKED_PASSWORD)
        self.assertEqual(
            StationStates("7f000000", 1), rainbird.get_available_stations()
        )
...
def mock_response(command, **kvargs):
    ...
    for k in resp:
        ...
        data = f"{data[:start_]}{(param_template % kvargs[k])}{data[end_:]}"

    add(
        POST,
        f"http://{MOCKED_RAINBIRD_HOST}/stick",
        body=encrypt(
